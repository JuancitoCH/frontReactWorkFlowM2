...
// import { loadingChange } from '../features/user/userSlice'
export default function Loader() {
    const {loading:loadingUser} = useSelector(state=>state.user)
    const {loading:loadingTeams} = useSelector(state=>state.teams)
    // const dispatch = useDispatch()
  return (
      <>
      {/* <button onClick={()=>dispatch(loadingChange())}>Loading</button> */}
      {(loadingUser || loadingTeams) &&
        <div className="flex justify-center items-center space-x-2 fixed z-50 ">
